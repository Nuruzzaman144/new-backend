...
const changeCurrentPassword=asyncHandler(async(req,res)=>{
  ...
  const user=await User.findById(req.user?._id)
   const isPasswordCorrectUser=await User.isPasswordCorrect(oldPassword)
...
const updateUserAvatar=asyncHandler(async(req,res)=>{
  ...
  return res
  .status(200)
  .json(new ApiResponse(200,user,"Cover image updated successfyully"))
})
...
const gatewatchHistory=asyncHandler(async(req,res)=>{
      ...
      $lookup:{
        ...
        pipeline:[
          ...
          },
          {
            $addFields:{
              owner:{
                $first:"$owner" ,  //array theke object
